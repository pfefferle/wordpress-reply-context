...
class ReplyContextPlugin {
	...
	public static function reply_context_meta_box( $post ) {
		...
		?>
		<p>
			<label for="reply_context_url"><?php _e( 'Send replies to:', 'reply_context' ); ?></label>
			<input type="text" name="reply_context_urls" id="reply_context_urls" class="code" value="<?php esc_attr( str_replace( PHP_EOL, ' ', $reply_context_urls ) ); ?>" style="width: 99%" />
			<br />
			(<?php _e( 'Separate multiple URLs with spaces' ); ?>)
		</p>
		<p>
			<?php _e( 'A "reply" is a kind of post that is in reply to some other post, that makes little or no sense without reading or at least knowing the context of the source post. Comments rarely (if ever) have names/titles, though they sometimes have other structure like multiple paragraphs, or blockquotes from the source that are being specifically responded too. More about <a href="https://indiewebcamp.com/reply">IndieWeb comments</a>!', 'reply_context' ); ?>
		</p>
		<?php
	}
	...
	public static function loop_start() {
		...
		foreach ( $reply_context_urls as $url ) {
			?>
			<cite class="p-in-reply-to h-cite">
				<a href="<?php echo esc_url( $url ); ?>" class="u-url"><?php echo $url; ?></a>
			</cite>
			<?php
		}
	}
